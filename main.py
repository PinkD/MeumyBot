...
def cmd_register(update: Update, context: CallbackContext):
    ...
    if token in tokens:
        ...
        chat_id = update.effective_chat.id
        print(f"{full_name}({username}) register callback for {chat_username}({chat_id}))")
        tokens.remove(token)
        chats.add(chat_id)
...
def cmd_unregister(update: Update, context: CallbackContext):
    ...
    if token in tokens:
        ...
        chat_username = update.effective_chat.username
        chat_id = update.effective_chat.id
        print(f"{full_name}({username}) unregister callback for {chat_username}({chat_id}))")
        tokens.remove(token)
...
def cmd_token(update: Update, context: CallbackContext):
    username = update.effective_user.username
    print(f"{username} try to generate token")
    if username in ADMIN_USERNAMES:
        token = gen_token()
        tokens.add(token)
        print(f"token generated by {username}")
        send_msg(update, context, f"one time token generated: `{token}`", md=True)
    else:
        print(f"denied for {username}")
